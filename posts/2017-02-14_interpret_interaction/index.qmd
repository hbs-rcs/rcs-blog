...
author: "Xiang Ao"
date: "2017-02-14"
categories: [statistics, Stata]
---

```{r}
#| echo: false
#| message: false
# knitr's built-in `engine = "stata"` does not work on Windows: StataMP-64.exe
# is a GUI application, so the call returns before Stata has written its log and
# knitr reads an empty file ("request ignored because of batch mode"). The
# Statamarkdown engine waits for Stata to finish, so the {stata} chunks below
# use it -- same as the specification-curve post.
library(Statamarkdown)

# Statamarkdown picks the newest Stata it finds. On the machine this was
# migrated on that is StataNow19, whose batch-mode graphics are broken:
# marginsplot fails with "twoway is not a valid graph subcommand" (r(198)) and
# writes corrupted bytes into the log, which then breaks knitr's UTF-8 decoding.
# Stata 17 renders the same code correctly. Set STATA_EXE to override; if the
# path below is absent we fall through to Statamarkdown's auto-detection.
stata_exe <- Sys.getenv("STATA_EXE", "C:/Program Files/Stata17/StataMP-64.exe")
if (file.exists(stata_exe)) {
  knitr::opts_chunk$set(engine.path = list(stata = stata_exe))
}
```
...
marginsplot
graph export marginsplot.png, replace width(1600)
```
