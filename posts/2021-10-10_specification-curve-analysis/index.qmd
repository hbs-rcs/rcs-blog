...
author: "Victoria L. Prince"
date: "2021-10-10"
categories: [reproducibility, Stata]
...
# Pin the Stata version, as in the other Stata posts: the newest install here
# (StataNow19) has broken batch-mode graphics. Set STATA_EXE to override; if the
# path below is absent, Statamarkdown's auto-detection takes over.
...
`speccurve` is a community-contributed command rather than part of Stata, so
the plotting step below is shown but not executed when the site is built. The
curve it produces is the figure that follows.
...
As above, the plotting step is shown but not executed:
